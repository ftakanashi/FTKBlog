...
def sync_read_count():
    ...
    :return:
    '''
    logger.info('Start to sync read_count from redis to db...')
    try:
        ...
        logger.error('Failed to sync: %s' % unicode(e))
        raise
    else:
        logger.info('read_count sync over.')
...
def refresh_today_access_count():
    ...
    :return:
    '''
    logger.info('Reset access count to 0...')
    try:
        r = get_redis_connection('default')
        r.set(settings.ACCESS_COUNT_KEY,0)
    except Exception,e:
        logger.error('Failed to reset access count: %s' % unicode(e))
    else:
